...
export default function AdminPropertyTypesPage() {
  ...
  const loadData = async () => {
    ...
    const response = await fetch(`/api/admin/property-types?${params.toString()}`, { credentials: "include" });
  ...
  const onToggleActive = async (item: PropertyTypeItem) => {
    const response = await fetch(`/api/admin/property-types/${item.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
  ...
  return (
    <section>
      <h1 className="text-xl font-bold">Property Types</h1>
      <p className="mt-1 text-xs text-slate-300">Master list with search/filter/pagination.</p>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1.25fr_1fr]">
        ...
        >
          ...
          <div className="flex flex-wrap items-end gap-2">
            <div className="min-w-[220px] flex-1">
              <label className="mb-1 block text-xs text-slate-400">Search type</label>
              <input
                value={search}
        ...
        <form
          ...
          className="rounded-xl border border-white/10 bg-slate-900/40 p-3"
        >
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Create Property Type</p>
          <div className="grid gap-2 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-xs text-slate-400">Type name</label>
              <input
                value={name}
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-400">Type slug</label>
              <input
                value={slug}
          ...
          <button className="mt-2 w-full rounded bg-emerald-600 px-3 py-2 text-sm font-semibold">Add Type</button>
        </form>
      </div>
